  ...
  return rows
    .map(booking => {
      ...
      return {
        ...
        duration: numberValue(meta.duration, SALON_DEFAULT_DURATION),
      };
    })
...
async function checkAvailability(args: ToolInput) {
  ...
  const technician = text(args.technician ?? args.stylist ?? args.staffName ?? args.tech);
  ...
  if (!duration) {
    ...
    if (serviceQuery) {
      const services = await db.select().from(demoCrmServices).where(eq(demoCrmServices.company, SALON_COMPANY));
      const match = findService(serviceQuery, services);
      duration = match?.durationMinutes ?? SALON_DEFAULT_DURATION;
    } else {
      duration = SALON_DEFAULT_DURATION;
    }
  }

  const technicians = technician
    ? [{ name: technician }]
    : await db.select({ name: demoCrmTechnicians.name }).from(demoCrmTechnicians).where(eq(demoCrmTechnicians.company, SALON_COMPANY));
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[salon-crm-availability] POST failed:', error);
