...
interface TradesAppointmentPayload {
...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json().catch(() => null)) as TradesAppointmentPayload | null;
  ...
  const description = [
    'Created from the Ephesus HVAC + Plumbing CRM demo.',
    '',
  ...
  try {
    ...
    console.error('[trades-crm] Failed to create Google Calendar event:', error);
